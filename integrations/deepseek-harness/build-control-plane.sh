...
plugin_root="$project_root/integrations/deepseek-harness/control-plane"
workspace_link="$harness_root/packages/extensions/mcp-control-plane"
...
# The bundle deliberately has no runtime imports from Harness packages; types
# are erased at build time. Remove the compatibility link created by older
# local builds so installation does not depend on pnpm's workspace layout.
if [[ -L "$plugin_root/node_modules" ]]; then
    unlink "$plugin_root/node_modules"
fi
pnpm --dir "$harness_root" install --no-frozen-lockfile \
    --filter @local/dsh-mcp-control-plane... \
    --network-concurrency=4 --child-concurrency=2 --reporter=append-only
pnpm --dir "$harness_root" --filter @local/dsh-mcp-control-plane bundle
test -f "$plugin_root/lib/index.mjs"
echo "Control-plane bundle ready: $plugin_root/lib/index.mjs"
